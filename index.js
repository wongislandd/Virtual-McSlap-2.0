...
client.on("guildCreate", (guild) => {
  ...
  if (guild.roles.cache.find((role) => role.name === "Scheduler") == null) {
    // sets the scheduler role to the custom created one by ID
    guild.roles
      .create({
        data: {
          name: "Scheduler",
...
client.on("message", async (msg) => {
  if (msg.channel.type == "dm") {
    // for pms, maybe reply but make sure it's not responding to itself.
    return console.log("A message was sent through a DM and has been ignored.");
  }
  if (msg.guild.id == COLLEGIATE_SERVER_ID && msg.author.id != CHRIS) {
    return;
  }
  ...
  switch (args[0]) {
    ...
    case "registerTeam":
      ...
      // Check if they have any scrims scheduled, if so, don't let them register until they're all gone.
    ...
    case "setTimezone":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !setTimeZone <Eastern/Pacific/Mountain/Central> \n**ex. !setTimeZone Eastern**"
        );
        return;
      }
    ...
    case "setAverageRank":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !setAverageRank Masters"
        );
        return;
      }
    ...
    case "post":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !post <Month> <Day> <Time> <AM/PM>\n**ex. !post May 20 5:00 PM**"
        );
        return;
      }
      if (!(await isAScheduler(msg))) {
    ...
    case "removeScheduler":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !removeScheduler <tag> **ex. !removeScheduler chriss#8261**"
        );
        return;
      }
      if (await isAScheduler(msg)) {
    ...
    case "changeOPGG":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !changeOPGG **ex. !changeOPGG https://na.op.gg/multi/query=wisperance%2Cbasu%2Csssssss**"
        );
        return;
      }
      if (await isAScheduler(msg)) {
    ...
    case "changeName":
      ...
      if (!msg.content.match(validInput)) {
        msg.reply(
          "The input was invalid. The correct format is !changeName **ex. !changeName Stony Brook Esports**"
        );
        return;
      }
      if (await isAScheduler(msg)) {
      ...
      break;
    case "setSchedulingChannel":
      if (await isAScheduler(msg)) {
        console.log(msg.guild.id, msg.channel.id)
        setChannelAsSchedulingChannel(msg, msg.guild.id, msg.channel.id)
      } else {
        msg.reply("Only Scheduler's can call this command!");
      }
      break;
    ...
    case "printSchedule":
      if (msg.author.id == CHRIS) {
        printScheduleByID(msg, args[1])
      }
      break;
    case "deleteListingByID":
      if (msg.author.id == CHRIS) {
        var validInput = /deleteListingByID[\s][\d+]/;
        if (!msg.content.match(validInput)) {
          msg.reply(
            "The input was invalid. The correct format is !deleteListingByID <ID>"
          );
          return;
        }
...
function botDescription(msg) {
  ...
  var configCommands = {
    // Config commands
    "!registerTeam":
      'Registers your teams discord within the bot\'s database. The correct format is !registerTeam "<team>" <opgg link> \n' +
    ...
    "!post":
      "Prepare to post a new scrim listing. The correct format is !post <Month> <Day> <Time> <AM/PM>",
    "!cancel":
      "Removes a **pending** scrim from your teams schedule by index. This will also remove it's public listing. The correct format is !cancel <index>",
    "!setTimezone":
      "Sets the desired time zone. The correct format is !setTimeZone <Eastern/Pacific/Central/Mountain>",
    "!changeOPGG":
      "Changes the team's OPGG link. The correct format is !changeOPGG <opgg>",
    "!changeName":
      "Changes the team's name. The correct format is !changeName <name>",
    "!makeMeAScheduler":
...
function rolesDescription() {
  var roles = {
    ...
    Scheduler:
      "A scheduler must have the @Scheduler role in the Discord and then register themselves to the bot with !makeMeAScheduler. A registered scheduler becomes the representative" +
    ...
    Player:
      "Users with the @Player role are tagged when a scrim is coming up soon.",
  };
...
function showServerSettings(msg) {
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      str += `\n**Player Role:** ${msg.guild.roles.cache.get(
        data.playerRoleID
      )}`;
      str += `\n**Scheduler Role:** ${msg.guild.roles.cache.get(
        data.schedulerRoleID
      )}`;
      str += `\n**Scheduling Channel:** ${msg.guild.channels.cache.get(
        data.schedulingChannelID
      )}`;
  ...
  db.collection("servers")
    ...
    .then(
      msg.channel.send(
        "```" +
          teamName +
          " successfully attatched to " +
          msg.guild.name +
          " within the database. Use !serverSettings to check what I know.```"
      )
    );
  console.log(
    teamName +
...
async function setChannelAsSchedulingChannel(msg, serverid, channelid){
  db.collection('servers').doc(serverid).update({
    schedulingChannelID : channelid
  })
  msg.reply("``` Successfully moved your scheduling channel. ```")
}
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      if (
        ...
        requestedTime.valueOf() < new Date().valueOf()
      ) {
      ...
      var formattedListing = Scrim.formatIntoConfirmationString(
        requestedTime,
        TIMEZONES[data.timeZone],
        data.team.name,
        data.team.schedulers,
        data.team.OPGG,
        data.team.avgRank
      );
...
function printSchedule(msg) {
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      Team.printSchedule(data, msg);
    });
}

function printScheduleByID(msg, serverid) {
  db.collection("servers")
    .doc(serverid)
    .get()
    .then((doc) => {
...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (
    ...
    reaction.message.content.includes("New Scrim Listing")
  ) {
    if (reaction.emoji.name == INTEREST_EMOJI) {
      ...
      if (awayServerID == "NOT FOUND") {
        user.send(
          "I see you reacted to a scrim listing but I couldn't find you registered under a team in my database! Please make sure you're registered."
        );
        return;
      } else {
        console.log(
          `${user.tag} belongs to ${client.guilds.cache.get(awayServerID).name}`
        );
        ...
        var firstListedSchedulerTag = content.substring(
          indexOfStart + 16,
          content.indexOf("#") + 5
        );
        ...
        console.log(
          `${firstListedSchedulerTag} belongs to ${
            client.guilds.cache.get(homeServerID).name
          }`
        );
        // Sends an Unknown Member error, not sure why. All behavior is as expected.
        showInterest(reaction, awayServerID, homeServerID, timeOfScrim).catch(
          (error) => error
        );
      }
    }
...
async function showInterest(
  ...
  var awayServerChannel = client.guilds.cache
    .get(awayServerID)
    .channels.cache.get(awaySchedulingChannelID);
  var homeServerChannel = client.guilds.cache
    .get(homeServerID)
    .channels.cache.get(homeSchedulingChannelID);
  homeServerChannel
    .send(
      "__**Scrim inquiry regarding the listing for " +
        timeOfScrim +
        ". React " +
        ACCEPT_EMOJI +
        " to accept or " +
        ...
        Team.teamAsString(
          awayTeamData.name,
          awayTeamData.team.avgRank,
          awayTeamData.team.schedulers,
          awayTeamData.team.OPGG
        )
    )
    .then(async (sentMsg) => {
      var filter = (reaction, user) => {
        return (
          ...
          user.id != BOTID &&
          isAScheduler2(awayServerID, user.id)
        );
      };
      ...
      sentMsg
        ...
        .then((collected) => {
          ...
          if (reaction.emoji.name === ACCEPT_EMOJI) {
            ...
            var indexOfScrim = findScrimIndexByTimeStr(
              homeTeamData.team.schedule,
              timeOfScrim
            );
            console.log(indexOfScrim);
            if (indexOfScrim == -1) {
              homeServerChannel.send(
                "Something went wrong. Please contact chriss#8261."
              );
              awayServerChannel.send(
                "Something went wrong. Please contact chriss#8261."
              );
              return;
            }
            ...
            homeServerChannel.send(
              "```" +
                `Accepted offer from ${awayTeamData.team.name} for ${timeOfScrim}` +
                "```"
            );
            awayServerChannel.send(
              "```" +
                `${homeTeamData.team.name} accepted your offer for ${timeOfScrim}` +
                "```"
            );
            ...
            homeServerChannel.send(
              "```" +
                `Declined offer from ${awayTeamData.team.name} for ${timeOfScrim}` +
                "```"
            );
            // Decline offer, notify away team.
            awayServerChannel.send(
              "```" +
                `${homeTeamData.name} declined your offer for ${timeOfScrim}` +
                "```"
            );
          }
        })
...
async function findSchedulingChannel(serverid) {
  ...
  return new Promise(function (resolve, reject) {
    db.collection("servers")
      ...
      .then(() => resolve("NOT FOUND, SHOULD NEVER HAPPEN."));
  });
}
...
async function findAssociatedTeam(schedulerTag) {
  ...
  return new Promise(function (resolve, reject) {
    db.collection("servers")
      .where("team.schedulers", "array-contains", schedulerTag)
      .get()
      .then((querySnapshot) => {
...
// Dev to delete any listing that may have bugged out.
function deleteListingByID(id){
  client.guilds.cache
  .get(COLLEGIATE_SERVER_ID)
  .channels.cache.get(COLLEGIATE_SCHEDULING_CHANNELID)
  .messages.fetch(id)
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      if (team.schedule[index].pending == false) {
        msg.reply(
          "This scrim is already confirmed. You must contact the opposing team if you wish to cancel."
        );
        return;
      }
...
async function removeConfirmedScrim(scrim) {
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      db.collection("servers").doc(homeTeamServerID).update({
        team: data.team,
      });
    })
    .catch((error) => console.log(error));
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      var index = findScrimIndexByTimeNum(schedule, scrim.time);
      schedule.splice(index, 1);
      db.collection("servers").doc(awayTeamServerID).update({
        team: data.team,
      });
    })
    .catch((error) => console.log(error));
...
async function confirmScrim(
  ...
  console.log(
    `Confirming scrim between ${homeTeamData.team.name} and ${awayTeamData.team.name}`
  );
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      db.collection("servers").doc(awayserverid).update({
        team: team,
      });
    })
    .catch((error) => console.log(error));
  db.collection("servers")
    ...
    .then((doc) => {
      ...
      var team = data.team;
      team.schedule.push(scrim);
      db.collection("servers").doc(homeserverid).update({
        team: team,
      });
    })
    .catch((error) => console.log(error));
...
function getPostingConfirmation(msg, formattedListing, data, timeValue) {
  var filter = (reaction, user) => {
    return (
      ...
      user.id != BOTID &&
      (user.id === msg.author.id || CHRIS)
    );
  };
  msg.channel
    .send("__Does this look good?__ \n" + formattedListing)
    .then(async (sentMsg) => {
      ...
      sentMsg
        ...
        .then((collected) => {
          ...
          if (reaction.emoji.name === CONFIRM_EMOJI) {
            msg.reply(
              "``` Listing posted to the Collegiate scheduling channel. ```"
            );
            client.guilds.cache
              .get(COLLEGIATE_SERVER_ID)
              .channels.cache.get(COLLEGIATE_SCHEDULING_CHANNELID)
              .send(
                `__New Scrim Listing. React ${INTEREST_EMOJI} to send a request.__ \n` +
                  formattedListing
              )
              .then(async (sentMsg) => {
...
function attemptToSetRoles(msg) {
  var schedulerRole = msg.guild.roles.cache.find(
    (role) => role.name === "Scheduler"
  );
  var playerRole = msg.guild.roles.cache.find((role) => role.name === "Player");
  if (schedulerRole == null) {
    msg.reply(
      "I tried to setup the Scheduler role as your primary scheduling role but I couldn't find it. \n Please create a role named Scheduler for me to use."
    );
  } else {
    db.collection("servers").doc(msg.guild.id).update({
  ...
  if (playerRole == null) {
    msg.reply(
      "I tried to setup the Player role as your primary player role but I couldn't find it. \n Please create a role named Player for me to use."
    );
  } else {
    db.collection("servers").doc(msg.guild.id).update({
...
function checkForDefaultFields(msg) {
  ...
  db.collection("servers")
    ...
    .then((doc) => {
      var str =
        "The following fields have been found to be uninitialized. You must intitialize them before continuing:";
      let data = doc.data();
...
async function checkNow() {
  ...
  docs.map(async (doc) => {
    ...
    var schedulingChannel = client.guilds.cache
      .get(doc.id)
      .channels.cache.get(data.schedulingChannelID);
    ...
    for (var i = 0; i < schedule.length; i++) {
      ...
      if (scrim.pending == true || scrimsRemovedThusFar.includes(scrim.msgID)) {
        if (timeOfScrim < currentTime) {
          console.log("Removing a past pending scrim.");
          client.guilds.cache
            .get(COLLEGIATE_SERVER_ID)
            .channels.cache.get(COLLEGIATE_SCHEDULING_CHANNELID)
            .messages.fetch(scrim.msgID)
          ...
          db.collection("servers")
            ...
            .then((doc) => {
              ...
              client.guilds.cache
                .get(doc.id)
                .channels.get(data.schedulingChannelID)
                .send("```One of your pending scrims have timed out.```");
              schedule.splice(i, 1);
              db.collection("servers").doc(doc.id).update({
                team: data.team,
              });
            });
        }
      ...
      if (timeUntilScrim <= 0) {
        schedulingChannel.send(
          `${playerRole} \n` +
            Scrim.formatIntoConfirmedString(
              data.team.name,
              timeOfScrim,
              TIMEZONES[data.timeZone],
              scrim.homeTeam,
              scrim.homeTeamSchedulers,
              scrim.homeTeamOPGG,
              scrim.homeTeamAvgRank,
              scrim.awayTeam,
              scrim.awayTeamSchedulers,
              scrim.awayTeamOPGG,
              scrim.awayTeamAvgRank
            ) +
      ...
      } else if (timeUntilScrim < 1800000 && timeUntilScrim > 900000) {
        // 15-30 min
        schedulingChannel.send(
          `${playerRole} \n` +
            Scrim.formatIntoConfirmedString(
      ...
      } else if (timeUntilScrim < 360000 && timeUntilScrim > 2700000) {
        // 45 - 60 min
        schedulingChannel.send(
          `${playerRole} \n` +
            Scrim.formatIntoConfirmedString(
  ...
  setInterval(async function () {
    ...
    docs.map(async (doc) => {
      ...
      for (var i = 0; i < schedule.length; i++) {
        ...
        if (
          scrim.pending == true ||
          scrimsRemovedThusFar.includes(scrim.msgID)
        ) {
          if (timeOfScrim < currentTime) {
            console.log("Removing a past pending scrim.");
            client.guilds.cache
        ...
        if (timeUntilScrim <= 0) {
          ...
          // Remove the scrim from both teams schedules
          await removeConfirmedScrim(scrim).catch((error) =>
            console.log(error)
          );
          console.log(`Pushing ${scrim.msgID} to array`);
          scrimsRemovedThusFar.push(scrim.msgID);
